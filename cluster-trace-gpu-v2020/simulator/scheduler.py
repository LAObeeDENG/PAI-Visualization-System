...
class Scheduler:
    def __init__(self, alloc_policy=0, preempt_policy=0, sort_node_policy=0,
                 cluster=None, gpu_type_matching=0, verbose=0):
    ...
    def alloc_job(self, cluster=None):
        ...
        job_list = cluster.job_list

        if len(job_list) <= 0:
            return 0
        ig, ic = cluster.idl_gpus, cluster.idl_cpus
        this_time_snapshot = [ig, ic, len(job_list), 0]
        if self.last_time_snapshot == this_time_snapshot:
            if self.verbose:
                print_fn("[{}] Last time snapshot == this time snapshot: {}. Bypass.".format(
                    self.cluster.cur_time, this_time_snapshot))
            return 0
        job_min_gpu = min(job_list, key=lambda j: j['num_inst'] * j['num_gpu'])
        job_min_cpu = min(job_list, key=lambda j: j['num_inst'] * j['num_cpu'])
        if (ig <= 0 or job_min_gpu['num_inst'] * job_min_gpu['num_gpu'] > ig) and \
           (ic <= 0 or job_min_cpu['num_inst'] * job_min_cpu['num_cpu'] > ic):
    ...
    def try_allocate_job_to_cluster(self, job_a, cluster):
        """
        return:
            -1: cluster is full, stop picking
             0: current job cannot be placed, try next
             1: job successfully deployed
        """
        ig, ic = cluster.idl_gpus, cluster.idl_cpus
        if ig <= 0 and ic <= 0:
            return -1
        elif job_a['num_inst'] * job_a['num_gpu'] > ig or \
             job_a['num_inst'] * job_a['num_cpu'] > ic:
            return 0
        else:
            assigned_node_map = {}
            assigned_inst_num = 0
            ...
            # --------------------------------------------------------
            # v100_gpu_planned：在 gang scheduling 的节点循环中，
            # 记录"本次调度已计划使用但尚未正式分配"的 V100 GPU 单位数。
            # 用来防止同一个 job 的多个 instance 重复占用池子配额。
            # --------------------------------------------------------
            v100_gpu_planned = 0
            ...
            for nid, node in enumerate(sorted_node_list):
                ...
                # ---- GPU 类型匹配（原有逻辑，不变）----
                if self.gpu_type_matching == 1:
                    if job_a['gpu_type'] != 'CPU' and job_a['gpu_type'] != node.gpu_type:
                        continue
                elif self.gpu_type_matching == 2:
                ...
                if job_a['num_inst'] == 1:
                    ...
                    if job_a['num_gpu'] <= node.idl_gpus and job_a['num_cpu'] <= node.idl_cpus:
                        ...
                        # ---- V100 预留池检查（新增）----
                        # 只在 V100 节点 且 reserve_ratio > 0 时生效
                        if node.gpu_type == 'V100' and cluster.reserve_ratio > 0:
                            is_large = (job_a.get('gpu_type') == 'V100')
                            gpu_need = job_a['num_gpu']

                            if is_large:
                                # 大任务：使用预留池
                                if cluster.reserved_v100_used + gpu_need > cluster.reserved_v100_gpus:
                                    continue  # 预留池已满，跳过此节点
                                # 通过检查：打标签 + 更新计数
                                job_a['v100_pool'] = 'reserved'
                                cluster.reserved_v100_used += gpu_need
                            else:
                                # 小任务：使用开放池
                                if cluster.open_v100_used + gpu_need > cluster.open_v100_gpus:
                                    continue  # 开放池已满，跳过此节点
                                job_a['v100_pool'] = 'open'
                                cluster.open_v100_used += gpu_need
                        # --------------------------------
                    ...
                    # ============ Gang scheduling（多实例任务）============
                    node_idle_gpus, node_idle_cpus = node.idl_gpus, node.idl_cpus
                    node_inst_num_gpu = job_a['num_inst']
                    node_inst_num_cpu = job_a['num_inst']
                    if job_a['num_gpu'] != 0:
                    ...
                    # ---- V100 预留池检查（新增）----
                    # 在确定能放多少个实例之后，再用池子容量进一步限制
                    if node.gpu_type == 'V100' and cluster.reserve_ratio > 0 and node_inst_num > 0:
                        is_large = (job_a.get('gpu_type') == 'V100')
                        if is_large:
                            available = (cluster.reserved_v100_gpus
                                         - cluster.reserved_v100_used
                                         - v100_gpu_planned)
                        else:
                            available = (cluster.open_v100_gpus
                                         - cluster.open_v100_used
                                         - v100_gpu_planned)

                        if job_a['num_gpu'] > 0:
                            # 按池子剩余容量限制可放实例数
                            pool_inst_limit = available // job_a['num_gpu']
                            node_inst_num = min(node_inst_num, pool_inst_limit)
                        # node_inst_num 可能变为 0，下面的 elif 会自动跳过
                    # --------------------------------

                    # 更新 assigned_node_map（原有逻辑，不变）
                    if assigned_inst_num + node_inst_num >= job_a['num_inst']:
                        ...
                        # 记录本节点计划用掉的 V100 GPU 量（封顶后的值）
                        if node.gpu_type == 'V100' and cluster.reserve_ratio > 0 and node_inst_num > 0:
                            v100_gpu_planned += node_inst_num * job_a['num_gpu']
                        break
            ...
            if assigned_inst_num < job_a['num_inst']:
                ...
                if self.cannot_counter % 100000 == 0:
                    print_fn("[%s] %d rejects. len(job_done_list) = %d. Current job: %s." % (
                        cluster.log_prefix, self.cannot_counter,
                        len(self.cluster.job_history.job_done_list),
                        _repr_job_concise(job_a)))
                return 0
            else:
                # 成功完成 gang scheduling，正式分配资源
                is_large = (job_a.get('gpu_type') == 'V100')
                pool_tag = 'reserved' if is_large else 'open'
                ...
                for nid, num_inst in assigned_node_map.items():
                    ...
                    for _ in range(num_inst):
                        ...
                        # ---- 打标签 + 更新 V100 池计数（新增）----
                        if node.gpu_type == 'V100' and cluster.reserve_ratio > 0:
                            job_tmp['v100_pool'] = pool_tag
                            if is_large:
                                cluster.reserved_v100_used += job_tmp['num_gpu']
                            else:
                                cluster.open_v100_used += job_tmp['num_gpu']
                        # ------------------------------------------
    ...
    def preempt_job(self, cluster=None):
        ...
        for job in preempted_job_list:
            ...
            # --------------------------------------------------------
            # 任务被抢占：归还 V100 池计数
            # 如果不归还，被抢占的任务虽然已经不在 V100 节点上运行，
            # 但计数器仍然占着，会导致池子提前耗尽（计数泄漏）。
            # 归还后重置 v100_pool 标签，等任务重新调度时再重新检查。
            # --------------------------------------------------------
            if cluster.reserve_ratio > 0 and job.get('v100_pool') is not None:
                if job['v100_pool'] == 'reserved':
                    cluster.reserved_v100_used -= job['num_gpu']
                    cluster.reserved_v100_used = max(0, cluster.reserved_v100_used)
                elif job['v100_pool'] == 'open':
                    cluster.open_v100_used -= job['num_gpu']
                    cluster.open_v100_used = max(0, cluster.open_v100_used)
                job['v100_pool'] = None  # 清除标签，重新调度时重新判断
            # --------------------------------------------------------
