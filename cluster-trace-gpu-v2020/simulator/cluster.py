...
class Cluster:
    def __init__(self, node_list=None, num_nodes=None, num_gpus=20,
                 ...
                 reserve_ratio=0.0):          # ← 新增参数：V100 预留比例
        ...
        # ============================================================
        # V100 预留池（Reservation Pool）
        #
        # 注意：node.num_gpus 内部单位是 实际GPU数 × 100
        # 例如一台 8-GPU 的 V100 节点，node.num_gpus = 800
        #
        # 两个池子：
        #   reserved pool：只有大任务（job['gpu_type']=='V100'）可用
        #   open pool    ：大任务和小任务都可用
        # ============================================================
        self.reserve_ratio = reserve_ratio

        # 统计所有 V100 节点的总 GPU 数（内部单位）
        self.total_v100_gpus = sum(
            node.num_gpus for node in node_list
            if node.gpu_type == 'V100'
        )

        # 按比例切分两个池子（内部单位）
        self.reserved_v100_gpus = int(self.total_v100_gpus * reserve_ratio)
        self.open_v100_gpus = self.total_v100_gpus - self.reserved_v100_gpus

        # 实时使用量计数器（内部单位，随任务分配/完成动态更新）
        self.reserved_v100_used = 0
        self.open_v100_used = 0

        print_fn(
            "V100 Pool Init: total=%d | reserved=%d (ratio=%.2f) | open=%d" % (
                self.total_v100_gpus,
                self.reserved_v100_gpus,
                reserve_ratio,
                self.open_v100_gpus,
            ), level=2
        )
        ...
        if len(job_runn_list) > 0:
            for job in job_runn_list:
                ...
                if job['on_time'] >= job['duration']:
                    ...
                    # ------------------------------------------------
                    # 任务完成：释放 V100 预留池计数
                    # job['v100_pool'] 由 scheduler 在分配时打上标签：
                    #   'reserved' → 来自预留池，计数归还给 reserved
                    #   'open'     → 来自开放池，计数归还给 open
                    #   None/不存在 → 不在 V100 节点上，无需处理
                    # ------------------------------------------------
                    if self.reserve_ratio > 0 and job.get('v100_pool') is not None:
                        if job['v100_pool'] == 'reserved':
                            self.reserved_v100_used -= job['num_gpu']
                            self.reserved_v100_used = max(0, self.reserved_v100_used)
                        elif job['v100_pool'] == 'open':
                            self.open_v100_used -= job['num_gpu']
                            self.open_v100_used = max(0, self.open_v100_used)
                    # ------------------------------------------------
            ...
            print_fn("%sIDLE cluster until jobs: %s" % (
                self.log_prefix, [_repr_job_preempt(e) for e in self.job_list]))
            if self.idle_cluster_counter % 10000 == 0:
                print_fn('{} idle cluster: {}'.format(
                    self.idle_cluster_counter,
                    [_repr_job_preempt(e) for e in self.job_list]), level=2)
            return self.cur_time

        elif len(self.job_full_list) > 0:
            wake_time = self.job_full_list[-1]['submit_time'] - delta
            assert self.cur_time <= wake_time
    ...
    def display_capacity_pattern(self, max_time=200):
        for cur_time in range(max_time):
            ...
            print("[%3s] G%3d |%s%s\n      C%3d |%s%s" % (
                cur_time, cur_gpus, "####|" * four_gpus, "#" * left_gpus,
                cur_cpus, "xxxx|" * four_cpus, "x" * left_cpus))
    ...
    def get_cap_ratio(self, time, pattern=None, period=None):
        ...
        pattern_ratio_dict = {
            ...
            5: {1: [(0, 10), (110, 124)], 0.9: [(10, 20), (100, 110)], 0.8: [(20, 30), (90, 100)],
                0.7: [(30, 40), (80, 90)], 0.6: (40, 50), 0.5: (50, 70), 0.4: (70, 80)},
    ...
    def log_prefix(self):
        if self.export_cluster_util is True:
            self.record_cluster_util()
        return "[%6s],[GPU,CPU]:[%7s,%8s]/[%7s,%8s]." % (
            self.cur_time, self.job_gpus, self.job_cpus,
            self.cur_gpus, self.cur_cpus)